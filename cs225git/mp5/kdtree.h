...
class KDTree
{
  ...
  private:
     ...
     */
    /*This function helps to select TreeNode at d dimension using Quick Select*/
    Point<Dim> quickSelect(vector<Point<Dim>>& newPoints, int left, int right, unsigned height,int k);
    // partition according to pivot
    int partition(vector<Point<Dim>>& newPoints, int left, int right, int pivotIndex, unsigned height);
    // a helper function for constructor to call
    void KDTreeHelper(vector<Point<Dim>>& list, int left, int right, int h, KDTreeNode *& root);
    // help to find nearest point
    Point<Dim> helpToFindNearest(const Point<Dim>& query, KDTreeNode* subroot, int height) const;
    //calculate in the distance in certain dim
    long dimVal(const Point<Dim>& first,
                             const Point<Dim>& second, int curDim) const;
    //calculate the distance between 2 point
    long distance(const Point<Dim>& target,
                               const Point<Dim>& currentBest) const;
    //helper function to delete whole tree
