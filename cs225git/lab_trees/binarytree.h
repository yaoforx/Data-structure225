...
class BinaryTree
{
  ...
  private:
    ...
    bool isBSTutil(struct Node *node, int min, int max) const;
    void mirror(Node* root) const;
    void printPaths(Node * root,vector<vector<T>> &paths, vector<int> row) const;
    int sumDistances(Node * root,int sum) const;
