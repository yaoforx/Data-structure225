...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    /* Iterator for where we want to insert the new child. 2 */
    auto child_itr = parent->children.begin() + child_idx + 1;
    /* Iterator for where we want to insert the new element. 1 */
    ...
    //std::cout<< "This is pop up "<< child->elements[mid_elem_idx].key<< std::endl;
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    typename std::vector<DataPair>::iterator it = subroot->elements.begin() + first_larger_idx;

    if(subroot->is_leaf) {
            subroot->elements.insert(it, pair); return;
    } else {
        insert(subroot->children[first_larger_idx], pair);
    }


    if(subroot->children.size() !=0 && subroot->children[first_larger_idx]->elements.size() >=order) {
