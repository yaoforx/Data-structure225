...
//    head_->nodePointers.push_back(SkipPointer(tail_,NULL));
    head_->nodePointers[0].next = tail_;
    tail_->nodePointers[0].prev = head_;
//    tail_->nodePointers.push_back(SkipPointer(NULL, head_));
...
SkipNode * SkipList::findR(int key) {
    SkipNode *rt = findRHelper(key, head_->nodePointers.size() - 1, head_);
